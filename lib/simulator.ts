...
export type SimResult = {
  ...
  payoffMultiple: { p5: number; p50: number; p95: number; mean: number };
  probInTheMoney: number;
  probDoubles: number;
  probTenX: number;
  probTargetMultiple: number;       // P(multiple >= targetMultiple)
  underlyingMoveForTarget: number;  // % move in underlying needed at expiry for option to hit targetMultiple
  leveragedMoveForTarget: number;   // % move in leveraged ETF needed at expiry
